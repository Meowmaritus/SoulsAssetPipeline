...
namespace SoulsAssetPipeline.Audio.Wwise
{
    ...
    {
        public class CAkAction : WwiseObjectBase<CAkAction>
        {
            ...
            public IWwiseObject ActionArgs;
            public uint RefID;
            ...
            public class ActArgs_Play : WwiseObjectBase<ActArgs_Play>
            {
            ...
            //Make each action have its own sub-structs and use conditional object for it, checking ActionType
            internal override bool CustomRead(BinaryReaderEx br, IWwiseObject parent)
            {
                ActionType = (ActionTypes)br.ReadUInt16();
                RefID = br.ReadUInt32();
                BitFlags = br.ReadByte();
                Props.Read(br, this);
                RangedProps.Read(br, this);

                ActionArgs = null;
                if (ActionType == ActionTypes.Play)
                {
                    ActionArgs = new ActArgs_Play();
                }

                if (ActionArgs != null)
                    ActionArgs.Read(br, this);

                //Parameters per-type here
                return true;
            }
