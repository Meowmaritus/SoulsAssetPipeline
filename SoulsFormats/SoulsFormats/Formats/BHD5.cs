    ...
    public class BHD5
    {
        ...
        public class FileHeader
        {
            ...
            internal FileHeader(BinaryReaderEx br, Game game)
            {
                ...
                UnpaddedFileSize = -1;
                if (game >= Game.DarkSouls3 && game != Game.EldenRing)
                {
            ...
            internal void Write(BinaryWriterEx bw, Game game, int bucketIndex, int fileIndex)
            {
                ...
                if (game == Game.EldenRing)
                {
                    ...
                    bw.WriteUInt32((uint)PaddedFileSize);
                    bw.WriteUInt32((uint)UnpaddedFileSize);
                }
                else
                {
                    bw.WriteUInt32((uint)FileNameHash);
                    bw.WriteInt32((int)PaddedFileSize);
                }
            ...
            public byte[] ReadFile(FileStream bdtStream)
            {
                ...
                if (PaddedFileSize > int.MaxValue)
                    throw new NotImplementedException("FUCKING REE I DONT FEEL LIKE FIGURING OUT HOW TO HANDLE THIS");
                bdtStream.Read(bytes, 0, (int)PaddedFileSize);
